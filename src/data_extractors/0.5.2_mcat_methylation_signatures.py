...
if __name__ == "__main__":
    ...
    if MODE == 'promoters':
        if TYPE == '27':
            ...
            print(f"Data has been correctly saved inside {table_paths[METHYLATION_SIGNATURES_PROMOTERS_27]} file")
        elif TYPE == '27full':
            methylation_dataframe.to_csv(table_paths[METHYLATION_SIGNATURES_PROMOTERS_27_FULL], index=False)
            print(f"Data has been correctly saved inside {table_paths[METHYLATION_SIGNATURES_PROMOTERS_27_FULL]} file")
        if TYPE == '450':
            methylation_dataframe.to_csv(table_paths[METHYLATION_SIGNATURES_PROMOTERS_450], index=False)
            print(f"Data has been correctly saved inside {table_paths[METHYLATION_SIGNATURES_PROMOTERS_450]} file")
        elif TYPE == '450full':
            methylation_dataframe.to_csv(table_paths[METHYLATION_SIGNATURES_PROMOTERS_450_FULL], index=False)
            print(f"Data has been correctly saved inside {table_paths[METHYLATION_SIGNATURES_PROMOTERS_450_FULL]} file")
    elif MODE == 'full':
        if TYPE == '27':
            methylation_dataframe.to_csv(table_paths[METHYLATION_SIGNATURES_FULL_27], index=False)
            print(f"Data has been correctly saved inside {table_paths[METHYLATION_SIGNATURES_FULL_27]} file")
        elif TYPE == '27full':
            methylation_dataframe.to_csv(table_paths[METHYLATION_SIGNATURES_FULL_27_FULL], index=False)
            print(f"Data has been correctly saved inside {table_paths[METHYLATION_SIGNATURES_FULL_27_FULL]} file")
        if TYPE == '450':
            methylation_dataframe.to_csv(table_paths[METHYLATION_SIGNATURES_FULL_450], index=False)
            print(f"Data has been correctly saved inside {table_paths[METHYLATION_SIGNATURES_FULL_450]} file")
        elif TYPE == '450full':
