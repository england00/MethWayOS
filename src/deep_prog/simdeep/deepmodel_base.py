...
class DeepBase(object):
    ...
    def fit_autoencoders(self, objective=None):
        ...
        for key in self.model_array:
            ...
            matrix_train = np.asarray(matrix_train)
            matrix_out = np.asarray(matrix_out)

            model.fit(x=matrix_train, y=matrix_out, verbose=verbose, epochs=self.epochs, validation_split=self.data_split, shuffle=True)
