...
class MultimodalCoAttentionTransformer(nn.Module):
    def __init__(self, meth_sizes: [], omic_sizes: [], model_size: str = 'medium', n_classes: int = 4, dropout: float = 0.25, fusion: str = 'concat', device: str = 'cpu'):
        ...
        omic_encoder_layer = nn.TransformerEncoderLayer(d_model=self.model_sizes[1], nhead=8, dim_feedforward=512, dropout=dropout, activation='relu')
    ...
    def forward(self, islands, omics, inference: bool = False):
        '''
        # WSI Fully connected layer
        # H_bag: (Mxd_k)
        H_bag = self.H(wsi).squeeze(0)
        '''
        # Methylation Fully connected layer
